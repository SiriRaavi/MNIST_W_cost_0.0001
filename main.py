...
def mann_train_cost():
    ...
    cost = T.mean(T.nnet.categorical_crossentropy(output_var_flatten, target_var.flatten())) + (0.0001 * l1(params[-2]))
    ...
    try:
        for i, (example_input, example_output, example_name) in generator:
            ...
            if i > 0 and not (i % MODEL_FREQ):
                f = open(save_dir + '/saved_models/train/model_' + str(i) + '.save', 'wb')
